...
///
/// - [DriveAdapter.userDrive] — full `drive` scope. Arbitrary base path in
///   the user's Drive. Use when files are written by multiple OAuth clients
///   (mobile app + CLI + Drive Desktop).
/// - [DriveAdapter.appFiles] — `drive.file` scope. Scoped to a folder the
///   app created. No CASA required.
/// - [DriveAdapter.appData] — `drive.appdata` scope. Hidden per-OAuth-client
///   folder. No CASA required.
class DriveAdapter implements StorageAdapter {
  ...
  factory DriveAdapter.userDrive({
    required http.Client httpClient,
    required String basePath,
    String? subPath,
  }) {
    final fullPath = PathValidator.joinBasePath(basePath, subPath);
    return DriveAdapter._(
  ...
  factory DriveAdapter.appFiles({
    required http.Client httpClient,
    required String folderName,
    String? subPath,
  }) {
    PathValidator.validateFolderName(folderName);
    PathValidator.validateSubPath(subPath);
  ...
  Future<String> _findOrCreateFolder(String name, String? parentId) async {
    final escapedName = _escapeDriveQuery(name);
  ...
  Future<void> uploadFile(String remotePath, List<int> content) async {
    ...
    final escapedPath = _escapeDriveQuery(remotePath);
    final query =
